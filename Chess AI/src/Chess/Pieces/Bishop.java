...
public class Bishop extends  Piece {
    private static int [] POSSIBLE_MOVES = {-9,-7,7,9};
    public Bishop(int position, PieceColor pieceColor) {
        super(position, pieceColor,PieceType.BISHOP);
    }
    ...
    private static boolean inFirstCol(int current, int offset) {
        return BoardUtils.COL_ONE[current] && ((offset == -9) || (offset == 7));
    }
    private static boolean inEighthCol(int current, int offset) {
        return BoardUtils.COL_ONE[current] && ((offset == -7) || (offset == 9));
    }
